...
const App: React.FC = () => {
  ...
  const [isCloudConfigured, setIsCloudConfigured] = useState(!!workspace.getConfig().gasUrl);
  ...
  if (!isCloudConfigured) {
    return (
      <div className="fixed inset-0 bg-[#0a0a0c] flex items-center justify-center p-6">
        <div className="glass w-full max-w-xl p-12 rounded-[3rem] border border-white/10 text-center space-y-8 animate-fade-in">
          ...
          <input 
            type="text" value={tempGasUrl} onChange={(e) => setTempGasUrl(e.target.value)}
            placeholder="Pega tu URL de Apps Script..." className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-xs text-white outline-none focus:border-blue-500"
          />
          <button onClick={handleConnectCloud} className="w-full bg-blue-600 py-4 rounded-2xl font-black text-xs uppercase tracking-widest">Conectar Omni Engine</button>
        </div>
      </div>
    );
  }
